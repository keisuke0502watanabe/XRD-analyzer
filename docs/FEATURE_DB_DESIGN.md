...
> **現状 (v23)**: Phase 1〜2 は実装済み。ピークフィット（擬フォークト）、Feature DB、Avrami
> ワークベンチ（誘導期 t₀ モード・積分窓・ベースラインモード・複数窓の合算・provenance 表示・
> CSV エクスポート）が入っている。以下の v15 / v16 の記述は設計当時の履歴として残す。
>
> **当時の現状**: v15 (`xrd_analyzer_v15.html`) は可視化・DB整理のみ。信号処理（平滑化・ベースライン・
